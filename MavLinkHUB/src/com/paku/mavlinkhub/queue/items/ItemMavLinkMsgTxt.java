...
public class ItemMavLinkMsgTxt {
	...
	public void setMe(ItemMavLinkMsg msgItem, MavLinkClassExtractor mavClasses) {
		...
		//		desc_1 = "System Id = [" + String.valueOf(msgItem.msg.sysid) + "]";
		desc_1 = msgItem.toString();
		desc_2 = "Sequences No. = [" + String.valueOf(msgItem.seqNo) + "]";
		desc_4 = "timeStamp = [" + String.valueOf(msgItem.timestamp) + "]";
		...
		case msg_heartbeat.MAVLINK_MSG_ID_HEARTBEAT:
			...
			mainTxt = ("Mode = " + mode.name() + " State = " + mavClasses.getMavState().get(msg_heartbeat_.system_status).getName());

			// ship's type name + autopilot name
			desc_3 = "ship's type name + autopilot name = " + (mavClasses.getMavType().get(msg_heartbeat_.type).getName() + ":" + mavClasses.getMavAutopilot().get(msg_heartbeat_.autopilot).getName());
